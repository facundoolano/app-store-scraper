...
function cleanList (results) {
  return results.feed.entry && results.feed.entry.map(function (app) {
    let developerUrl, developerId;
    if (app['im:artist'].attributes) {
      developerUrl = app['im:artist'].attributes.href;
      const firstPart = app['im:artist'].attributes.href.split('/id')[1];
      developerId = firstPart && firstPart.split('?mt')[0];
    }
...
function list (opts) {
  return new Promise(function (resolve, reject) {
    ...
    const options = Object.assign({
      url: `https://itunes.apple.com/${opts.country}/rss/${opts.collection}/limit=${opts.num}${category}/json`,
      method: 'GET'
    }, opts.requestOptions);
